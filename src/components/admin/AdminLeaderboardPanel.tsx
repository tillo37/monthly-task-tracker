import { useCallback, useEffect, useState } from 'react';
import { RefreshCw } from 'lucide-react';
import type { LeaderboardSource } from '../../data/leaderboardSource';
import { METRIC_LABELS, type LeaderboardEntry, type LeaderboardMetric } from '../../lib/leaderboard';
import { addMonths, currentMonthKey, monthLabel } from '../../lib/date';
import { formatDuration } from '../../lib/time';
import { formatCount } from '../../lib/admin';

type Period = 'thisMonth' | 'lastMonth';

/**
 * The leaderboard as an administrator sees it — which is the same leaderboard
 * everybody else sees. It calls the same two RPCs, so there is no privileged
 * variant returning emails or sessions; the only thing the panel adds is the
 * ability to flip between this month and last without leaving the page.
 */
export function AdminLeaderboardPanel({
  source,
  currentUserId,
}: {
  source: LeaderboardSource;
  currentUserId: string | null;
}) {
  const [metric, setMetric] = useState<LeaderboardMetric>('time');
  const [period, setPeriod] = useState<Period>('thisMonth');
  const [entries, setEntries] = useState<LeaderboardEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const month =
    period === 'thisMonth' ? currentMonthKey() : addMonths(currentMonthKey(), -1);

  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      setEntries(await source.fetch(metric, month));
    } catch (failure) {
      setError(failure instanceof Error ? failure.message : 'Could not load the leaderboard.');
    } finally {
      setLoading(false);
    }
  }, [metric, month, source]);

  useEffect(() => {
    void load();
  }, [load]);

  return (
    <section className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold tracking-tight">Leaderboard</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {monthLabel(month)} · the same aggregates the public board shows
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <div
            role="group"
            aria-label="Ranking metric"
            className="flex rounded-lg border border-slate-200 p-0.5 dark:border-slate-700"
          >
            {(['time', 'completions'] as const).map((option) => (
              <button
                key={option}
                type="button"
                aria-pressed={metric === option}
                onClick={() => setMetric(option)}
                className={`btn h-8 px-3 ${
                  metric === option
                    ? 'bg-indigo-600 text-white'
                    : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
                }`}
              >
                {METRIC_LABELS[option]}
              </button>
            ))}
          </div>

          <div
            role="group"
            aria-label="Leaderboard month"
            className="flex rounded-lg border border-slate-200 p-0.5 dark:border-slate-700"
          >
            {(
              [
                ['thisMonth', 'This month'],
                ['lastMonth', 'Previous month'],
              ] as const
            ).map(([option, label]) => (
              <button
                key={option}
                type="button"
                aria-pressed={period === option}
                onClick={() => setPeriod(option)}
                className={`btn h-8 px-3 ${
                  period === option
                    ? 'bg-indigo-600 text-white'
                    : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
                }`}
              >
                {label}
              </button>
            ))}
          </div>

          <button
            type="button"
            className="btn btn-icon btn-subtle"
            aria-label="Refresh the leaderboard"
            onClick={() => void load()}
          >
            <RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} aria-hidden="true" />
          </button>
        </div>
      </header>

      {error && (
        <p role="alert" className="card p-4 text-sm text-red-600 dark:text-red-400">
          {error}
        </p>
      )}

      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <caption className="sr-only">
              {METRIC_LABELS[metric]} for {monthLabel(month)}, highest first
            </caption>
            <thead>
              <tr className="border-b border-slate-200 text-left text-[11px] font-semibold tracking-wide text-slate-500 uppercase dark:border-slate-800 dark:text-slate-400">
                <th scope="col" className="w-16 px-4 py-2.5">Rank</th>
                <th scope="col" className="px-4 py-2.5">User</th>
                <th scope="col" className="px-4 py-2.5 text-right">
                  {metric === 'time' ? 'Total time' : 'Completions'}
                </th>
                {metric === 'time' && (
                  <th scope="col" className="px-4 py-2.5 text-right">Sessions</th>
                )}
              </tr>
            </thead>
            <tbody>
              {entries.map((entry) => (
                <tr
                  key={entry.userId}
                  className={`border-b border-slate-100 last:border-0 dark:border-slate-800/60 ${
                    entry.userId === currentUserId ? 'bg-indigo-50/60 dark:bg-indigo-500/10' : ''
                  }`}
                >
                  <td className="px-4 py-2.5 font-semibold tabular-nums">{entry.rank}</td>
                  <td className="px-4 py-2.5 font-medium">{entry.displayName}</td>
                  <td className="px-4 py-2.5 text-right font-semibold tabular-nums">
                    {metric === 'time'
                      ? formatDuration(entry.totalSeconds)
                      : formatCount(entry.completionCount)}
                  </td>
                  {metric === 'time' && (
                    <td className="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-400">
                      {entry.sessionCount}
                    </td>
                  )}
                </tr>
              ))}

              {entries.length === 0 && !loading && (
                <tr>
                  <td
                    colSpan={metric === 'time' ? 4 : 3}
                    className="px-4 py-10 text-center text-sm text-slate-500 dark:text-slate-400"
                  >
                    Nobody tracked anything in {monthLabel(month)}.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </div>
    </section>
  );
}
